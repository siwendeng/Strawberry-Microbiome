...
#### 
CAP_table<-read.table("CAPS_table_SampleTypebyTimepoint.txt",header = T)
...
CAP_table$Factor<-factor(CAP_table$Factor,levels=c("Treatment","Replicate"))
...
CAPS_plot_bray<-ggplot(data=CAP_table_bray,aes(x=Factor,y=PercVariance,fill=log10(pvalue))) + 
  ...
  geom_errorbar(aes(ymin=PercVariance-se, ymax=PercVariance+se), width=.1, position=position_dodge(0.1)) +
...
CAPS_plot_bray
ggsave(filename = "CAPs_plot_bray_sampletypeandtp.pdf",plot=CAPS_plot_bray,width=4.5,height=5.5)
...
CAPS_plot_unifrac<-ggplot(data=CAP_table_unifrac,aes(x=Factor,y=PercVariance,fill=log10(pvalue))) + 
  ...
  scale_fill_gradient(low="black", high="grey")
CAPS_plot_unifrac
ggsave(filename = "CAPs_plot_wUF_sampletypeandtp.pdf",plot=CAPS_plot_unifrac,width=4.5,height=5.5)
