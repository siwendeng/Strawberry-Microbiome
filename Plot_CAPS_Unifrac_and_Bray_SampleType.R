...
#### 
CAP_table<-read.table("CAPS_table_SampleType.txt",header = T)
CAP_table$SampleType<-factor(CAP_table$SampleType,levels=c("Soil","Rhizosphere","Root"))
CAP_table$Factor<-factor(CAP_table$Factor,levels=c("Treatment","Timepoint","Replicate"))
...
CAPS_plot_bray
ggsave(filename = "CAPs_plot_bray.pdf",plot=CAPS_plot_bray,width=4.5,height=5.5)
...
CAPS_plot_unifrac<-ggplot(data=CAP_table_unifrac,aes(x=Factor,y=PercVariance,fill=log10(pvalue))) + 
  ...
  scale_fill_gradient(low="black", high="grey")
CAPS_plot_unifrac
ggsave(filename = "CAPs_plot_wUF.pdf",plot=CAPS_plot_unifrac,width=4.5,height=5.5)
